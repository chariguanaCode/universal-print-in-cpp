...
template <typename T> using is_iterable = decltype(detail::is_iterable_impl<T>(0)); //determines if variable is iterable
...
template <typename T, typename std::enable_if< (     is_iterable  <T>::value),T>::type* =nullptr> void array_process(T &x, std::queue<int> sizes); //for writing dimension
 ...
 *  Colours!
 * ------------------------------------------------------------------------------ */
std::string colour(int val)
{
    if(ANSIsupport)
        return "\033[38;5;"+std::to_string(val)+"m";
    return "";
}
std::string backgr(int val)
{
    if(ANSIsupport)
        return "\033[48;5;"+std::to_string(val)+"m";
    return "";
}
std::string bold  ()
{
